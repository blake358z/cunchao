...
OUT_MD = "村超MiniApp产品PRD.md"
OUT_DOCX = "村超MiniApp产品PRD.docx"
...
figma_file_url = "https://www.figma.com/design/AKoXl1o98NcM8b3F7SCADS"

ui_screens = [
    ("EXPORT_01_HOME_平台首页", "首页", "浅色资讯流首页；增加新闻列表、互动数据、可下滑内容流", "31:3", "ui_exports_v2/EXPORT_01_HOME_平台首页.png"),
    ("EXPORT_02_EVENTS_赛事频道", "赛事频道", "横滑联赛卡；轻按筛选赛程，二次点击进入联赛首页", "31:64", "ui_exports_v2/EXPORT_02_EVENTS_赛事频道.png"),
    ("EXPORT_03_LEAGUE_联赛首页", "联赛首页", "补齐赛程、排行榜小标题，强化联赛内导航", "31:135", "ui_exports_v2/EXPORT_03_LEAGUE_联赛首页.png"),
    ("EXPORT_04_MATCH_比赛详情", "比赛详情", "比分、事件流、互动评论入口；点击比赛进入", "31:195", "ui_exports_v2/EXPORT_04_MATCH_比赛详情.png"),
    ("EXPORT_05_TEAM_球队详情", "球队详情", "弱化大头图，改为资讯/赛程/球员/讨论/球队信息五个 tab", "31:247", "ui_exports_v2/EXPORT_05_TEAM_球队详情.png"),
    ("EXPORT_06_TRAVEL_文旅服务", "文旅服务", "按比赛预约入场，按村寨/球队探索文旅攻略", "31:312", "ui_exports_v2/EXPORT_06_TRAVEL_文旅服务.png"),
    ("EXPORT_07_COMMUNITY_社区", "社区", "帖子卡补齐点赞、评论、收藏数据", "31:365", "ui_exports_v2/EXPORT_07_COMMUNITY_社区.png"),
    ("EXPORT_08_PROFILE_我的关注", "我的", "新增我的评论、点赞、收藏、浏览历史沉淀", "31:418", "ui_exports_v2/EXPORT_08_PROFILE_我的关注.png"),
    ("EXPORT_09_ARTICLE_资讯详情", "资讯详情", "资讯二级页，包含正文、点赞、评论、收藏和热门评论", "31:472", "ui_exports_v2/EXPORT_09_ARTICLE_资讯详情.png"),
    ("EXPORT_10_POST_帖子详情", "帖子详情", "帖子二级页，包含主帖、评论、点赞、回复", "31:519", "ui_exports_v2/EXPORT_10_POST_帖子详情.png"),
    ("FLOW_核心用户路径", "用户路径", "首页资讯、赛事、球队、社区、文旅、我的行为沉淀闭环", "31:571", "ui_exports_v2/FLOW_核心用户路径.png"),
]
...
def md():
    ...
    for title, paras in sections:
        ...
        lines.append("")
        if title == "4. 产品范围":
            add_md_table(lines, "MVP模块优先级", tables["MVP模块优先级"])
        if title == "5. 多联赛频道布局策略":
            add_md_table(lines, "多联赛频道结构", tables["多联赛频道结构"])
        if title == "8. 数据源策略":
            add_md_table(lines, "数据源分级", tables["数据源分级"])
        if title == "9. 后端与数据模型":
            add_md_table(lines, "核心实体", tables["核心实体"])
        if title == "12. 风险与待确认问题":
            add_md_table(lines, "开放问题清单", tables["开放问题清单"])
        if title == "14. UI与交互优化说明":
            add_md_table(lines, "关键交互规则", tables["关键交互规则"])
            add_md_table(lines, "球队页Tab规则", tables["球队页Tab规则"])
            add_md_table(lines, "我的页面沉淀", tables["我的页面沉淀"])
        if title == "15. UI设计稿与页面对应":
            lines.append("### 页面设计图索引")
            lines.append("| 帧名/文件名 | 对应模块 | 说明 | Figma 节点 | 本地图片 |")
            lines.append("|---|---|---|---|---|")
            for frame, module, note, node_id, path in ui_screens:
                if node_id == "待同步":
                    node_link = "待同步"
                else:
                    node_url = f"{figma_file_url}?node-id={node_id.replace(':','-')}"
                    node_link = f"[{node_id}]({node_url})"
                lines.append(f"| {frame} | {module} | {note} | {node_link} | `{path}` |")
            lines.append("")
            lines.append("### 页面预览")
            for frame, module, note, node_id, path in ui_screens:
                lines.append(f"#### {module}｜{frame}")
...
def add_md_table(lines, title, data):
    lines.append(f"### {title}")
...
def set_cell(cell, text, header=False):
    cell.text = ""
    p = cell.paragraphs[0]
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER if header or len(text) <= 12 else WD_ALIGN_PARAGRAPH.LEFT
    p.paragraph_format.space_after = Pt(0)
    r = p.add_run(text)
    set_font(r, 8.5 if len(text) > 28 else 9, bold=header)
    cell.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER
    if header:
        shade(cell, "E8EEF5")


def add_doc_table(doc, title, data):
    p = doc.add_paragraph()
    p.paragraph_format.space_before = Pt(6)
    p.paragraph_format.space_after = Pt(4)
    r = p.add_run(title)
    set_font(r, 11, bold=True, color=(31, 58, 95))
    table = doc.add_table(rows=len(data), cols=len(data[0]))
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    table.style = "Table Grid"
    for i, row in enumerate(data):
        for j, txt in enumerate(row):
            set_cell(table.cell(i, j), txt, header=(i == 0))
    doc.add_paragraph()
...
def build_docx():
    ...
    r = title.add_run("村超 Mini App 产品需求文档（PRD）")
    set_font(r, 24, color=(0, 0, 0))

    sub = doc.add_paragraph()
    sub.paragraph_format.space_after = Pt(12)
    r = sub.add_run("版本：v0.1｜日期：2026-06-13｜阶段：立项 / MVP 定义")
    set_font(r, 9.5, color=(85, 85, 85))

    note = doc.add_paragraph()
    note.paragraph_format.left_indent = Inches(0.12)
    note.paragraph_format.space_after = Pt(8)
    r = note.add_run("本 PRD 以微信小程序为首发形态，同时保证业务后端、数据模型、内容后台和设计系统可复用于后续独立 APP。")
    set_font(r, 9.5, color=(67, 67, 67))
    r.italic = True
    ...
    for title_text, paras in sections:
        ...
        for p_text in paras:
            ...
            r = p.add_run(p_text)
            set_font(r, 9.5)
        if title_text == "4. 产品范围":
            add_doc_table(doc, "MVP模块优先级", tables["MVP模块优先级"])
        if title_text == "5. 多联赛频道布局策略":
            add_doc_table(doc, "多联赛频道结构", tables["多联赛频道结构"])
        if title_text == "8. 数据源策略":
            add_doc_table(doc, "数据源分级", tables["数据源分级"])
        if title_text == "9. 后端与数据模型":
            add_doc_table(doc, "核心实体", tables["核心实体"])
        if title_text == "12. 风险与待确认问题":
            add_doc_table(doc, "开放问题清单", tables["开放问题清单"])
        if title_text == "14. UI与交互优化说明":
            add_doc_table(doc, "关键交互规则", tables["关键交互规则"])
            add_doc_table(doc, "球队页Tab规则", tables["球队页Tab规则"])
            add_doc_table(doc, "我的页面沉淀", tables["我的页面沉淀"])
        if title_text == "15. UI设计稿与页面对应":
            add_doc_table(doc, "页面设计图索引", [["帧名/文件名", "模块", "说明", "Figma节点"]] + [[a,b,c,d] for a,b,c,d,_ in ui_screens])
            for idx, (frame, module, note, node_id, path) in enumerate(ui_screens):
                p = doc.add_paragraph()
                p.paragraph_format.space_before = Pt(8)
                p.paragraph_format.space_after = Pt(4)
                r = p.add_run(f"{module}｜{frame}")
                set_font(r, 11, bold=True, color=(31, 58, 95))
                p2 = doc.add_paragraph()
                r2 = p2.add_run(f"{note}；Figma 节点：{node_id}；本地图片：{path}")
                set_font(r2, 8.8, color=(85, 85, 85))
                try:
                    width = Inches(6.4) if frame.startswith("FLOW") else Inches(2.35)
                    doc.add_picture(path, width=width)
                except Exception as e:
                    p3 = doc.add_paragraph()
                    r3 = p3.add_run(f"[图片插入失败：{path}，{e}]")
                    set_font(r3, 9, color=(155, 28, 28))

    doc.add_heading("参考来源", level=1)
    for name, url, note in sources:
        p = doc.add_paragraph(style="List Bullet")
        r = p.add_run(f"{name}：{note}。{url}")
        set_font(r, 8.8)
